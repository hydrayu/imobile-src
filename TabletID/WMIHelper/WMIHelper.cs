...
namespace WMIHelper
{
    public static class WMIHelper
    {
        ...
        public static Serial[] FillSerial(string machineName, string userName, string password,
                                                          string queryArea, string filter="")
        {
            InitializeScope(machineName, userName, password);
            ManagementClass managementClass = new ManagementClass(path + ":" + queryArea);
            if (mScope != null)
            ...
            if (filter != "")
            {
                ...
                {
                    if (m["Caption"].ToString().Contains(filter)) count++;
                }
            }
            else count = instances.Count;
            
            Serial[] serialPorts = new Serial[count];
            ManagementObjectCollection.ManagementObjectEnumerator enumerator = instances.GetEnumerator();
            int i = 0;
            while (enumerator.MoveNext())
            {
                if (filter == "" || enumerator.Current["Caption"].ToString().Contains(filter))
                {
                    serialPorts[i] = new Serial(enumerator.Current);
                    i++;
                }
            }
            return serialPorts;
