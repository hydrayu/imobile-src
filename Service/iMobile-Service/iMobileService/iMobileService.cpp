...
BOOL LaunchAppIntoDifferentSession(char *exe_path, int diffSession);
...
#define DEF_LOG_FILE "C:\\iMobileService.log"
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
  ...
  for (int i=0; i<launchers; i++) {
	  LOG("Call LaunchAppIntoDifferentSession(%s)", launcherCmds[i]);
	  LaunchAppIntoDifferentSession(launcherCmds[i], 1);
  }
  bRunning=true;
  while(bRunning)
  {
...
void WINAPI ServiceCtrlHandler(DWORD Opcode)
{
  ...
  case SERVICE_CONTROL_CUSTOM_MESSAGE:
	  ...
	  LaunchAppIntoDifferentSession(triggerCmd, 0); 
	  break; 
...
BOOL LaunchAppIntoDifferentSession(char *exe_path, int diffSession) 
{
   ...
   char sessionCmd[2][30] = { "explorer.exe", "winlogon.exe" };
...
////////////////////////////////////////
   ...
   PROCESSENTRY32 procEntry;
	...
	if(diffSession != 0) diffSession = 1;

	do
	{
		if (_stricmp(procEntry.szExeFile, sessionCmd[diffSession]) == 0)
		{
