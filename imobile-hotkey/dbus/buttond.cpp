...
int db_send (DBusConnection *dbconn, char* msg)
{
  ...
  dbmsg = dbus_message_new_signal("/imobile/buttonsd/Object", // object name of the signal
    "imobile.buttonsd.Type", // interface name of the signale
    "Pressed"); // name of the signal
  if (!dbmsg) goto err;
  if (!dbus_message_append_args(dbmsg, DBUS_TYPE_STRING, &msg, DBUS_TYPE_INVALID)) goto err;
  if (!dbus_connection_send(dbconn, dbmsg, NULL)) goto err;
