...
/*
  這支相當棒，至少有底下幾點理由:
  1. 程式碼清楚，簡單
  2. 有完整範例, 意思是可以用現有的 Visual Studio 2012 來編譯
  3. 示範的 dll (InjectDll.dll)
  4. 事實上，更重要的是，使用上相當簡單，有彈性
  5. 範例中 WinMain() 雖然寫的是叫用外部的 exe, 但是可以做到嵌入 Process 中
*/
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	// 但是要配合將 ResumeThread() 拿掉
	// CREATE_SUSPENDED 的話，被呼叫的應用會被停住
	...
	// ResumeThread(pi.hThread); // 搭配 前方 CreateProcess(CREATE_SUSPENDED) 使用
...
#else
/*
底下示範的是如何嵌入一個 dll 到 winlogon process ,
比較特別的是，它嵌入的就是 process.....
*/
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
