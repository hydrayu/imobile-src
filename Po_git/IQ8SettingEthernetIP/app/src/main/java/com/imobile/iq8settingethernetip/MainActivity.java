...
public class MainActivity extends Activity {
	...
	private int Po_write_file(String IP, String Mask, String Gateway,
		...
		try {
			...
			bw.write("     fi\n");
			bw.write("     \n");
			bw.write("     ndc resolver setifdns eth0 \""+domain+"\" " + DNS + " " + DNS2
					+ "\n");
	...
	public void Po_save_config(View view) {
		...
		if (this.Read_IP().equals("X")) {
			...
			Toast.makeText(MainActivity.this, "DNS2 format is error!",
					Toast.LENGTH_SHORT).show();
		} else if (this.Read_Domain().equals("X")) {
			Toast.makeText(MainActivity.this, "Search Domain format is error!",
					Toast.LENGTH_SHORT).show();
		} else {
	...
	/***
	 * Po_And	 is a button, can read current IP information
	 * 
	 * @param view
	 */
