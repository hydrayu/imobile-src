...
//===========================================================================
{
	...
	// wade, parse command line
	m_comport = "COM3";
	LPTSTR CommandLineDetails = AfxGetApp()->m_lpCmdLine;
	CString sParam(CommandLineDetails);
	if (sParam.Left(3) == "/p:") {
		m_comport = sParam.Right(sParam.GetLength() - 3);
	}
}
...
//===========================================================================
{
	...
	case  CREATEPORT:
		...
		// wade, Connect to default COM Port
		if(!m_UARTComm.COMPortMC(m_comport) && !m_UARTComm.COMPortAC())
		{
