...
public class VncCanvasActivity extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		if ((data != null) && (data.getScheme().equals("vnc"))) {
		...
		else if ((data != null) && (data.getScheme().equals("myVnc"))) {
			String host = data.getHost();
			...
			connection.setAddress(host);
			connection.setNickname("localhost");
			connection.setPort(port);
			List<String> path = data.getPathSegments();
			if (path.size() >= 1) {
				String color = path.get(0);
				if (color.contains("256"))
					connection.setColorModel(COLORMODEL.C256.nameString());
				else if (color.contains("64"))
				...
				else if (color.contains("reyscale") || color.equals("C4"))
					connection.setColorModel(COLORMODEL.C4.nameString());
				else if (color.contains("lack") || color.equals("C2"))
