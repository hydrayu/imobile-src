...
namespace WebcamSecurity
{
    public partial class MainForm : Form
    {
        ...
        #region DLL Import 宣告
        #region ACamDll Import 宣告....舊晶片/大板子
        ...
        #endregion
        #region SMIUtility.dll import 宣告
        [DllImport("SMIUtility.dll")]
        public static extern int SMI_HowManySMIDevices();
        [DllImport("SMIUtility.dll")]
        public static extern int SMI_CreateDeviceObject();
        [DllImport("SMIUtility.dll")]
        public static extern int SMI_DestoryDeviceObject();
        [DllImport("SMIUtility.dll")]
        public static extern int SMI_GetVIDPIDByIdx(int[] iVID, int[] iPID, int idx);
        #endregion
        #endregion
        ...
        private void log(Byte[] baMsg, string prev, int mode)
        {
            ...
            else if (mode == 1)
            {
                ...
                {
                    if (letter == 0x00) break;
                    sLog += String.Format("{0:X2} ", letter);
                }
            }
            log(prev + " '" + sLog + "'");
        }
        ...
        private string CamVid(string source)
        {
            return source.Substring(source.IndexOf(string.Format("vid_"))+4, 4);
        }

        private string CamPid(string source)
        {
            return source.Substring(source.IndexOf(string.Format("pid_"))+4, 4);
        }
        ...
        private void SetFocus(int camIndex)
        {
            ...
            startCamera();
            // Camera : Vid: 090c, Pid: f37d
            string vid = CamVid(CamMonitor[activeCamera].cam.Source);
            string pid = CamPid(CamMonitor[activeCamera].cam.Source);
            log("Camera : Vid: " + vid + ", Pid: " + pid);
            if (vid == "090c" && pid == "f37d")
            {
        ...
        //if (ALC_GetFirmwareVersion(baALC))
        //{
        //    log(baALC, "ALC_GetFirmwareVersion() return", 1);
        //}
        //else
        ...
        **/
        public void LED_Enable(int mode)
        {
            ...
            if (mode > 0)
            {
                ...
                buttonLED.BackColor = System.Drawing.Color.Yellow;
                log(data, "LED on data ", 1);
            }
            else
            {
                // assign GPIO
                ...
                data[0] &= (Byte)(~LedOn);
                ALC_ProcWriteToISP(LedLoc, xLen, data);
                buttonLED.BackColor = System.Drawing.Color.Brown;
                log(data, "LED off data ", 1);
            }
        }

        private void buttonLED_Click(object sender, EventArgs e)
        {
            if (!bLED) LED_Enable(1);
            else       LED_Enable(0);
            bLED = !bLED;
        }
    }
